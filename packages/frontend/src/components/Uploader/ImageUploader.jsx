...
export default function ImageUploader({ setImageUrls, imageUrls }) {
    ...
    async function uploadImages() {
        ...
        try {
            ...
            const promises = images.map(image => uploadSingleImage(client, image.file))
            ...
            >
                {({
                    ...
                    <div className="upload__image-wrapper">
                        <div className='cursor-pointer' style={isDragging ? { color: 'red' } : undefined}
                        ...
                        {imageList.map((image, index) => (
                            <div key={index} className="image-item">
                                <img src={image['data_url']} alt="" className="object-cover h-48 w-96" />
                                <div className="image-item__btn-wrapper">
                                    <button className="inline-flex items-center w-full px-5 py-1 mb-3 text-base font-semibold text-white no-underline align-middle bg-green-600 border border-transparent border-solid rounded-md cursor-pointer select-none sm:mb-0 sm:w-auto hover:bg-green-700 hover:border-green-700 hover:text-white focus-within:bg-green-700 focus-within:border-green-700" onClick={() => onImageUpdate(index)}>Update</button>
                                    <button className="inline-flex items-center w-full px-5 py-1 mt-3 text-base font-semibold text-white no-underline align-middle bg-red-600 border border-transparent border-solid rounded-md cursor-pointer select-none sm:mb-0 sm:w-auto hover:bg-red-700 hover:border-red-700 hover:text-white focus-within:bg-red-700 focus-within:border-red-700" onClick={() => onImageRemove(index)}>Remove</button>
                                </div>
                            </div>
                        ))}
                        {images.length > 0 &&
                            <div>
                                <button onClick={onImageRemoveAll} className="inline-flex items-center w-full px-5 py-3 mb-3 mr-1 text-base font-semibold text-white no-underline align-middle bg-red-600 border border-transparent border-solid rounded-md cursor-pointer select-none sm:mb-0 sm:w-auto hover:bg-red-700 hover:border-red-700 hover:text-white focus-within:bg-red-700 focus-within:border-red-700">
                                    Remove all Images
                        ...
                        {uploading && <span className="text-green-600 underline">Uploading...</span>}
